...
true_OD_dir = os.path.join(main_dir, "true_OD.pkl")
...
def write_empty_routes_file(output_dir, vtype_id="commonType"):
    routes = ET.Element(
        "routes",
        {
    ...
    empty_path = os.path.join(output_dir, "empty.rou.xml")
...
def step_simulation(router, od_pairs, od_vec, det_ids, step_idx, veh_counter_start, sim_time_start):
    ...
    for sec in range(input_interval):
        ...
        if (sim_time % detect_interval) == 0:
            ...
            interval_det_data = [float(libsumo.inductionloop.getLastIntervalVehicleNumber(did)) for did in det_ids]
...
det_ids = []
traci_started = False
temp_dir = tempfile.mkdtemp(prefix="dode_ground_truth_")
...
try:
    ...
    for step_idx in range(num_step):
        ...
        if interval_det_data is not None:
            print(f"[t={sim_time}] 300s interval detector nVehContrib: {interval_det_data}")
...
csv_path = os.path.join(main_dir, "answer.csv")
