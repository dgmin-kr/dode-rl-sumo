...
class TimeDependentShortestPathRouter:
    def __init__(
        ...
        self._edge_index = {}
        self._node_index = {}
        self._node_out_edge_indices = {}
        self._edge_len_arr = None
        self._edge_vmax_arr = None
        self._edge_to_node_idx = None
        self._edge_out_indptr = None
        self._edge_out_indices = None
        self._edge_rank = None
    ...
    def warmup(self):
        cache_key = (os.path.abspath(self.net_file), bool(self.exclude_internal_edges), "edge_index_v1")
        ...
        if cached is not None:
            (
                ...
                self._edge_len_arr,
                self._edge_vmax_arr,
                self._edge_to_node_idx,
                self._edge_out_indptr,
                self._edge_out_indices,
                self._edge_rank,
            ) = cached
        ...
        for e in net.getEdges():
            ...
            edge_ids.append(eid)
            self.node_out_edges.setdefault(u, []).append(eid)
            self.edge_to_node[eid] = v
            self.edge_len[eid] = float(e.getLength())
            self.edge_vmax[eid] = float(e.getSpeed())

            outs = []
            out_map = e.getOutgoing()
            for to_edge in out_map.keys():
                tid = to_edge.getID()
                if self.exclude_internal_edges and tid.startswith(":"):
                    continue
                outs.append(tid)
            self.edge_out[eid] = outs

        self._edge_ids = edge_ids
        self._edge_index = {eid: i for i, eid in enumerate(edge_ids)}
        self._node_index = {nid: i for i, nid in enumerate(node_ids)}
        self._node_out_edge_indices = {
            nid: np.asarray([self._edge_index[eid] for eid in eids if eid in self._edge_index], dtype=np.int64)
            for nid, eids in self.node_out_edges.items()
        }

        n_edges = len(edge_ids)
        self._edge_len_arr = np.empty(n_edges, dtype=np.float64)
        self._edge_vmax_arr = np.empty(n_edges, dtype=np.float64)
        self._edge_to_node_idx = np.empty(n_edges, dtype=np.int64)
        self._edge_rank = np.empty(n_edges, dtype=np.int64)
        for eid, idx in self._edge_index.items():
            self._edge_len_arr[idx] = self.edge_len[eid]
            self._edge_vmax_arr[idx] = self.edge_vmax[eid]
            self._edge_to_node_idx[idx] = self._node_index[self.edge_to_node[eid]]
        for rank, eid in enumerate(sorted(edge_ids)):
            self._edge_rank[self._edge_index[eid]] = rank

        counts = [len(self.edge_out.get(eid, [])) for eid in edge_ids]
        self._edge_out_indptr = np.empty(n_edges + 1, dtype=np.int64)
        cursor = 0
        for i, count in enumerate(counts):
            self._edge_out_indptr[i] = cursor
            cursor += count
        self._edge_out_indptr[n_edges] = cursor
        self._edge_out_indices = np.empty(cursor, dtype=np.int64)
        cursor = 0
        for eid in edge_ids:
            for next_eid in self.edge_out.get(eid, []):
                idx = self._edge_index.get(next_eid)
                if idx is not None:
                    self._edge_out_indices[cursor] = idx
                    cursor += 1
        if cursor != self._edge_out_indices.shape[0]:
            self._edge_out_indices = self._edge_out_indices[:cursor].copy()
            cursor = 0
            for i, eid in enumerate(edge_ids):
                self._edge_out_indptr[i] = cursor
                for next_eid in self.edge_out.get(eid, []):
                    if next_eid in self._edge_index:
                        cursor += 1
            self._edge_out_indptr[n_edges] = cursor
    ...
    def _shortest_time_route_edges_python(self, O: str, D: str):
        ...
        start_edges = self.node_out_edges.get(O, [])
        if not start_edges:
            return None
        ...
        for se in start_edges:
            w = self._edge_travel_time(se)
            dist[se] = w
            prev[se] = None
            heapq.heappush(pq, (w, se))

        best_end = None

        while pq:
            d_u, e_u = heapq.heappop(pq)
            if e_u in visited:
                continue
            visited.add(e_u)

            if self.edge_to_node.get(e_u) == D:
                best_end = e_u
                break

            for e_v in self.edge_out.get(e_u, []):
                w = self._edge_travel_time(e_v)
                nd = d_u + w
                if (e_v not in dist) or (nd < dist[e_v]):
                    dist[e_v] = nd
                    prev[e_v] = e_u
                    heapq.heappush(pq, (nd, e_v))
    ...
    def shortest_time_route_edges(self, O: str, D: str):
        if O == D:
            return None
        start_edges = self._node_out_edge_indices.get(O)
        dest_node = self._node_index.get(D, -1)
        if start_edges is None or start_edges.size == 0 or dest_node < 0:
            return None
        if self._use_numba and self._edge_out_indptr is not None:
            try:
                edge_weight = self._build_travel_time_array()
                path_idx = _dijkstra_edge_indices(
                    start_edges,
                    int(dest_node),
                    self._edge_to_node_idx,
                    self._edge_out_indptr,
...
class MySumoEnv(gym.Env):
    ...
    def _detlog(self, msg: str):
        p = os.path.join(self.rl_dir, "dump", "detector_debug.log")
    ...
    def _start_sumo(self, seed: Optional[int] = None):
        ...
        all_det_ids = self.det_ids
        self._detlog("========== DETECTOR DISCOVERY ==========")
        self._detlog(f"[ALL] n={len(all_det_ids)} ids={all_det_ids}")

        self.det_ids, self._det_group_slices, self._det_base_ids = Config.build_detectors_and_groups(
            all_det_ids
        )

        for base, (s, e) in zip(self._det_base_ids, self._det_group_slices):
            self._detlog(f"[MAP] base={base} slice=({s}:{e}) ids={self.det_ids[s:e]}")

        if getattr(Config, "DETECTOR_STRICT", False):
            missing = [base for base, (s, e) in zip(self._det_base_ids, self._det_group_slices) if e == s]
            if missing:
                raise RuntimeError(f"Some base detector ids have 0 matches in TraCI: {missing}. "
                                f"Check detectors.add.xml naming / DETECTOR_ID_SEP.")
        ...
        self._det_block_sum = np.zeros(len(self.det_ids), dtype=np.float32)
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None) -> Tuple[np.ndarray, dict]:
        ...
        det_state = np.array([0] * self.num_det, dtype=np.float32)
    ...
    def step(self, action: np.ndarray):
        ...
        a  = a0 * float(self.action_unit_veh)
        ...
        net_state = np.asarray(net_state, dtype=np.float32)
        det_state = Config.aggregate_detector_vector(self._det_block_sum, self._det_group_slices)
        ...
        if (self.current_step % interval_steps) == 0:
            reward = float(r_acc)
        ...
        if (self.current_step % interval_steps) == 0:
            ...
            if (last_det is not None) and (self._det_group_slices is not None):
                last_det = Config.aggregate_detector_vector(last_det, self._det_group_slices)
            self._last_det_data = None
    ...
    def _compute_reward(self, t: int):
        ...
        if self._det_group_slices is None:
            raise RuntimeError("Detector group slices are not initialized. Check _start_sumo().")
        det = Config.aggregate_detector_vector(det, self._det_group_slices)

        ans = self._read_answer_data((t + 1) // interval_steps - 1)
        if ans.size == 0:
            return 0.0, 0.0

        if ans.size != det.size:
            raise ValueError(
                f"answer.csv column count(ans.size={ans.size}) != aggregated detector dim(det.size={det.size}). "
                f"Check Config.DETECTOR_BASE_IDS / answer.csv."
            )

        if os.environ.get("SUMO_DEBUG_REWARD") == "1":
            print(f"[ANS]: {ans}")
            print(f"[DET]: {det}")
